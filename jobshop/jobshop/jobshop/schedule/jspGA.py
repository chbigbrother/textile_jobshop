...
from .GAOperations import checkDiversity, generate_population, evolve
...
def genetic(times, machines, n, mn, population_number, iterations, rate, target):
    machine_number = mn # len(machines[0])
    ...
    def sortAndGetBestIndividual(population):
        ...
        for individual in population:
            result = None
            if not individual[1]:
                result, table = calculateMakespan(times, machines, individual[0], n, mn)
                individual[1] = result
            else: 
                result = individual[1]
    ...
    best_result, best_table = calculateMakespan(times, machines, global_best_ind[0], n, mn)

    result = plotResult(best_table, best_result)
